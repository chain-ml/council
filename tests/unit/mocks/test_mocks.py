...
class TestPrompt(unittest.TestCase):
    def test_empty(self):
        m = MockLLM()
        r = m.post_chat_request(LLMContext.new_empty(), [])
    ...
    def test_from_message(self):
        ...
        r = m.post_chat_request(LLMContext.new_empty(), [LLMMessage.user_message("Test")])
        self.assertEqual("Test", r.first_choice)

    def test_token_limit(self):
        m = MockLLM(action=llm_message_content_to_str, token_limit=3)
        with self.assertRaises(LLMTokenLimitException):
            _ = m.post_chat_request(LLMContext.new_empty(), [LLMMessage.user_message("Test")])
